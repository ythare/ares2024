...
icp_localization::icp_localization() : private_node_("~"), tf_listener_(tfBuffer_)
{
    // \033[1;32m，\033[0m 终端显示成绿色
    ROS_INFO_STREAM("\033[1;32m----> ICP location started.\033[0m");
    // ros::console::set_logger_level(ROSCONSOLE_DEFAULT_NAME, ros::console::levels::Debug);

    laser_scan_subscriber_ = nh.subscribe("/scan_filtered", 1, &icp_localization::ScanCallback, this, ros::TransportHints().tcpNoDelay());
    map_subscriber_ = nh.subscribe("map", 1, &icp_localization::MapCallback, this);
    odom_subscriber_ = nh.subscribe("/odom", 20, &icp_localization::OdomCallback, this, ros::TransportHints().tcpNoDelay());
 
    map_pointcloud_publisher_ = nh.advertise<sensor_msgs::PointCloud2>("map_pointcloud", 10);  //map 转化为PointCloud2的结果可视化
    icp_pointcloud_publisher_ = nh.advertise<PointCloudT>("icp_pointcloud", 1, this);  //可视化每次匹配得到的点云数据
    relocate_tranform_visuial_publisher_ = nh.advertise<geometry_msgs::PoseWithCovarianceStamped>("relocate_visuial_pose", 1, this);//重定位结果可视化
    relocate_initialpose_publisher_ = nh.advertise<geometry_msgs::PoseWithCovarianceStamped>("initialpose", 1, this); //重定位结果
    icp_pointcloud_publisher_ = nh.advertise<PointCloudT>("icp_pointcloud", 1, this);  //发布匹配结果
    location_info_publisher_ = nh.advertise<roborts_msgs::LocationInfo>("location_info", 1, this);  //定位信息
    rotate_pointcloud_publisher_ = nh.advertise<PointCloudT>("rotate_pointcloud", 1, this);
    rotate_robotpose_publisher_ = nh.advertise<geometry_msgs::PoseWithCovarianceStamped>("rotate_robot_pose", 1, this);

    location_publisher_ = nh.advertise<geometry_msgs::PoseWithCovarianceStamped>("location_match", 1, this);

    relocalization_srv_ = nh.advertiseService("relocalization", &icp_localization::RelocalizeCallback, this);
 ...
 * 的参数初始化
 */
void icp_localization::InitParams()
{
    private_node_.param<bool>("if_debug", if_debug_, true);
    private_node_.param<bool>("save_pcd", save_pcd_, false);
    ...
    private_node_.param<std::string>("lidar_frame", lidar_frame_, "livox_frame");
 ...
 * Odom回调函数
 */
void icp_localization::OdomCallback(const nav_msgs::Odometry::ConstPtr &odometry_msg)
{
    ...
    // std::cout<< "linear x :" << odometry_msg->pose.pose.position.x<<std::endl;
    // std::cout<< "linear y :" << odometry_msg->pose.pose.position.y<<std::endl;
    // std::cout<< "angular z:" << odometry_msg->twist.twist.angular.z<<std::endl;
 ...
 * Map回调函数 进行数据处理
 */
void icp_localization::MapCallback(const nav_msgs::OccupancyGrid::ConstPtr& map_msg)
{
...
* scan 回调函数
*/
void icp_localization::ScanCallback(const sensor_msgs::LaserScan::ConstPtr &scan_msg)
{
   scan_start_time_ = std::chrono::steady_clock::now();    //保存时间，用于最后计时
   //判断地图和里程计数据是否初始化，如果没有则退出
    if (!map_initialized_ || !odom_initialized_)  
    {
        ROS_ERROR("-------map or odom is not initialized---------<waiting>");
        return;
    }
    //计算起始时间
    ...
    if (!scan_initialized_)
    {
        ...
        std::cout << "map_to_lidar_ :  \n" << map_to_lidar_.matrix() << std::endl;
    }
    else
    {
        if (if_debug_){
    ...
    if(need_relocalization){
        ...
        // step1 进行数据类型转换
        start_time_ = std::chrono::steady_clock::now();
        Scan2PointCloud(scan_msg, cloud_scan_);
        end_time_ = std::chrono::steady_clock::now();
        time_used_ = std::chrono::duration_cast<std::chrono::duration<double>>(end_time_ - start_time_);
        if (if_debug_){
 ...
 * 重定位服务回调函数
*/
bool icp_localization::RelocalizeCallback(roborts_msgs::Relocate::Request& req, roborts_msgs::Relocate::Response& res)
{
    ros::Rate rate(10);
    // Set the relocalization flag to true
    need_relocalization = true;
    ROS_INFO("---------relocalization service start");
    // 等待目标话题完成    
 ...
 */
void icp_localization::OccupancyGridToPointCloud(const nav_msgs::OccupancyGrid::ConstPtr &map_msg, PointCloudT::Ptr &cloud_msg)
{
 ...
 */
void icp_localization::Scan2PointCloud(const sensor_msgs::LaserScan::ConstPtr &scan_msg, PointCloudT::Ptr &cloud_msg)
{
...
bool icp_localization::ScanMatchWithICP(Eigen::Isometry3d &trans , PointCloudT::Ptr &cloud_scan_msg, PointCloudT::Ptr &cloud_map_msg)
{
...
*/
bool icp_localization::ReLocationWithICP(Eigen::Isometry3d &trans ,const sensor_msgs::LaserScan::ConstPtr &scan_msg, PointCloudT::Ptr &cloud_map_msg, const Eigen::Isometry3d &robot_pose)
{
    //将雷达和地图点云保存为pcd文件
    if (save_pcd_){
        // 获取当前时间并格式化成字符串
        ...
        std::string scan_cloud_filename = std::string("/home/ares_yt/ws_livox/src/scan2map/PCD_File/scan_cloud_") + buffer + ".pcd";
        std::string map_cloud_filename = std::string("/home/ares_yt/ws_livox/src/scan2map/PCD_File/map_cloud_") + buffer + ".pcd";
 ...
 */
bool icp_localization::GetTransform(Eigen::Isometry3d &trans , const std::string parent_frame, const std::string child_frame, const ros::Time stamp)
{
 ...
 * trans 类型： Eigen::Isometry3d
 */
bool icp_localization::Get2TimeTransform(Eigen::Isometry3d &trans)
{
 ...
 * trans 类型： Eigen::Isometry3d
 */
bool icp_localization::GetOdomTransform(Eigen::Isometry3d &trans, double start_time, double end_time)
{
 ...
 * 对点云数据绕机器人坐标点进行旋转
*/
void icp_localization::rotatePointCloud(PointCloudT::Ptr &cloud_msg, const Eigen::Affine3f &rotation, const Eigen::Affine3f &robo_pose)
{
    //将点云转换到原点
...
geometry_msgs::PoseWithCovarianceStamped icp_localization::Isometry3d_to_PoseWithCovarianceStamped(const Eigen::Isometry3d& iso)
{
 ...
 * 对点云进行体素滤波剔除离群点
 */
void icp_localization::PointCloudVoxelGridRemoval(PointCloudT::Ptr &cloud_msg, double leafSize)
{
 ...
 */
void icp_localization::PointCloudObstacleRemoval(PointCloudT::Ptr &cloud_map_msg, PointCloudT::Ptr &cloud_msg, double Distance_Threshold)
{
...
bool icp_localization::is_coordinate_in_range(const std::vector<double>& vec, const Eigen::Isometry3d &coord)
{
    if(vec.size() %4 != 0){
